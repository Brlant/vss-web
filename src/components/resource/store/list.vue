...
<template>
  <div>
            ...
            <ul class="show-list">
              <li v-for="item in showTypeList" :class="{'active':item.id==currentItem.id}" class="list-item"
                ...
                <div class="id-part">
                  ...
                  <span class="pull-right">
                    <el-tag v-show="item.status==='3'" type="danger">停用</el-tag>
                    <el-tag v-show=" item.status==='0'" type="success">正常</el-tag>
                </span>
                </div>
                <div>
                  {{item.name }}
        ...
        <div :style="'height:'+bodyHeight" class="d-table-col-wrap">
          ...
          <div v-else>
            ...
            <div class="page-main-body">
              <oms-row :span="4" label="仓库名称">
                {{data.name}}
              </oms-row>
              <oms-row :span="4" label="仓库类型">
                {{warehouseType}}
              </oms-row>
              <oms-row v-if="data.warehouseType==='0'" :span="4" label="所属物流公司">
                {{data.warehouseSourceFirmName}}
              </oms-row>
              <oms-row v-show="data.logisticsCentreId" :span="4" label="物流中心">
                {{ data.logisticsCentreName}}
              </oms-row>
              <oms-row :span="4" label="仓库地址类型">
                <dict :dict-group="'orgAddress'" :dict-key="formatAddress"></dict>
              </oms-row>
              <oms-row :span="4" label="所在地区">
                {{ storeAddress }}
              </oms-row>
              <oms-row :span="4" label="详细地址">
                {{ data.detail }}
              </oms-row>
              <oms-row :span="4" label="联系人">
                {{ data.contact }}
              </oms-row>
              <oms-row :span="4" label="联系电话">
                {{ data.telephone }}
              </oms-row>
            </div>
          </div>
    ...
    <page-right :show="showRight" @right-close="resetRightBox">
      <store-part :actionType="showRight" :formItem="formItem" :formType="formType" @change="onSubmit"
