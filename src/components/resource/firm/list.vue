...
<template>
  <div>
      ...
      <div class="container d-table">
        ...
        <div class="d-table-left">
          ...
          <div :style="'height:'+ (bodyHeight - 60)  + 'px'" class="d-table-col-wrap" @scroll="scrollLoadingData">
            ...
            <div v-else>
              <ul class="show-list">
                <li v-for="item in businessRelationList" :class="{'active':item.id==currentItem.id}" class="list-item"
                    @click="showType(item)">
                  <!--<div class="id-part">-->
                ...
                <div class="base-pic-list">
                  <div v-for=" licence in businessRelationItem.followOrg.licenses" class="base-pic-item">
                    <div @click="watchPhoto(licence)">
                      ...
                      <div>
                        <el-tag v-show="licence.expireStatus==='0'" type="primary">正常</el-tag>
                        <el-tag v-show="licence.expireStatus==='1'" type="warning">即将到期</el-tag>
                        <el-tag v-show="licence.expireStatus==='2'" type="danger">已过期</el-tag>
                      </div>
                    </div>
                  </div>
                </div>
    ...
    <page-right :show="showRight" @right-close="resetRightBox">
      <el-form ref="relationForm" :model="form" :rules="rules" class="demo-ruleForm" label-width="100px"
               onsubmit="return false" @submit.prevent="onSubmit('relationForm')">
        <h2 class="clearfix">{{!form.id ? '添加厂商' : '修改厂商'}}</h2>
        <el-form-item label="往来单位" prop="followOrgId">
          <el-select v-model="form.followOrgId" :clearable="true" :remote-method="queryOtherBusiness" filterable
                     placeholder="请输入名称搜索厂商"
                     popper-class="good-selects" remote>
            <el-option v-for="item in orgList" v-show="item.auditedStatus ==1" :key="item.id" :label="item.name"
                       :value="item.id">
        ...
        <el-form-item label-width="120px">
          <el-button :disabled="doing" native-type="submit" type="primary" @click="onSubmit('relationForm')">保存
          </el-button>
