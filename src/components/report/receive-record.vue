  ...
  <div class="order-page">
    <div class="container">
      ...
      <el-table ref="reportTable" v-loading="loadingData" :data="reportList" :header-row-class-name="'headerClass'"
        ...
        <el-table-column label="供货单位" min-width="150px" prop="saleFilmName"></el-table-column>
        <el-table-column label="疫苗名称" min-width="200px" prop="vaccineName"></el-table-column>
        <el-table-column label="规格" min-width="150px" prop="specification"></el-table-column>
        <el-table-column label="生产厂家" min-width="150px" prop="factoryName"></el-table-column>
        <el-table-column :sortable="true" label="数量" prop="count" width="80px"></el-table-column>
        <el-table-column label="疫苗批号" min-width="120" prop="batchNumber"></el-table-column>
...
export default {
  ...
  methods: {
    exportFile: function () {
      ...
      this.$http.get('/erp-receiving-record/export?time=' + this.time).then(res => {
        if (res.data.url) {
    ...
    search: function () {// 搜索
      ...
      this.$http.get('/erp-receiving-record/query?time=' + this.time).then(res => {
        ...
        this.reportList = [];
        this.$notify.error({
          message: error.response && error.response.data && error.response.data.msg || '查询失败'
        });
      });
    },
