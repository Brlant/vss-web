    ...
    <div class="content-part">
      ...
      <div class="content-right min-gutter">
        ...
        <div class="hide-content" v-bind:class="{'show-content' : index==0}">
          <el-form ref="addForm" :model="form" :rules="rules" label-width="100px" onsubmit="return false"
            ...
            <el-form-item label="收款单位" prop="orgId">
              <el-select v-model="form.orgId" :clearable="true" :remote-method="filterOrg" filterable placeholder="请输入名称搜索收款单位"
                         remote @click.native.once="filterOrg('')">
                <el-option v-for="org in orgList" :key="org.remitteeId" :label="org.remitteeName"
                           :value="org.remitteeId">
            ...
            <el-form-item label="付款方式" prop="payType">
              <el-select v-model="form.payType" placeholder="请选择付款方式">
                <el-option v-for="item in PaymentMethod" :key="item.key" :label="item.label" :value="item.key"/>
              </el-select>
            </el-form-item>
            <el-form-item label="付款金额" prop="amount">
              <oms-input v-model="form.amount" disabled placeholder="请选择付款明细，自动计算总额" type="text" @blur="formatPrice">
                <template slot="prepend">¥</template>
              </oms-input>
            </el-form-item>
            <el-form-item label="说明">
              <oms-input v-model="form.explain" :autosize="{ minRows: 2, maxRows: 5}" placeholder="请输入备注信息"
                         type="textarea"></oms-input>
            </el-form-item>
          </el-form>
        </div>
        <div class="hide-content" v-bind:class="{'show-content' : index==1}">
          <pay-detail :amount="form.amount" :billPayType="form.billPayType"
                      :factoryId="form.orgId" :selectPayments="selectPayments"></pay-detail>
        </div>
      </div>
    </div>
  </div>
