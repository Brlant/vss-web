...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      audited: function () {
        this.$confirm('确认通过付款作业申请"' + this.form.no + '"的审核?', '', {
          ...
          BillPayable.auditInfo(this.form.id, {
            ...
            this.$notify.success({
              duration: 2000,
              title: '成功',
              message: '审核付款作业申请"' + this.form.no + '"成功'
            });
            ...
            this.$notify.error({
              duration: 2000,
              message: '审核付款作业申请"' + this.form.no + '"失败'
            });
          });
        });
      },
      notAudited: function () {
        this.$confirm('确认不通过付款作业申请"' + this.form.no + '"的审核?', '', {
          confirmButtonText: '确认',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          BillPayable.refusedInfo(this.form.id, {auditOpinion: this.form.auditOpinion}).then(() => {
            this.$notify.success({
              duration: 2000,
              title: '成功',
              message: '付款作业申请"' + this.form.no + '"审核未通过'
            });
            this.$emit('change', this.form);
            this.$emit('right-close');
          }).catch(() => {
            this.$notify.error({
              duration: 2000,
              message: '付款作业申请"' + this.form.no + '"审核未通过失败'
            });
          });
        });
      }
