    ...
    <div class="container">
      <div class="opera-btn-group" :class="{up:!showSearch}">
        <div class="opera-icon">
          <perm label="erp-stock-inventory-add" class="opera-btn pull-right">
            <span style="cursor:pointer" @click.prevent="add">
              <a href="#" @click.prevent="" class="btn-circle"><i class="el-icon-t-plus"></i></a> 增加</span>
          </perm>
        ...
        <el-form v-show="showSearch" class="advanced-query-form" onsubmit="return false">
          <el-row>
            <el-col :span="6">
              <oms-form-row label="操作人" :span="6">
                <el-select filterable remote clearable placeholder="请输入名称搜索" :remote-method="filterOperators"
                           @focus="filterOperators('')" v-model="filters.createdBy">
                  <el-option :value="o.id" :key="o.id" :label="o.name" v-for="o in operators"></el-option>
                </el-select>
              </oms-form-row>
            </el-col>
            <el-col :span="10">
      ...
      <div class="order-list clearfix" style="margin-top: 20px">
        ...
        <div v-else class="order-list-body flex-list-dom">
          <div class="order-list-item order-list-item-bg" v-for="item in inventories"
               :class="[{'active':currentOrderId===item.id}]" @click.prevent="showDetail(item)">
            <el-row>
              ...
              <el-col :span="6" class="opera-btn">
                <perm label="erp-stock-inventory-delete">
                    <span @click.stop="deleteDetail(item)">
                       <a href="#" class="btn-circle" @click.prevent=""><i class="el-icon-t-delete"></i></a>删除
                    </span>
                </perm>
