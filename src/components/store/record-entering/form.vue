      ...
      <div class="content-left">
        ...
        <ul>
          <li class="text-center" style="margin-top:40px;position:absolute;bottom:30px;left:0;right:0;">
            <el-button :disabled="doing" @click="onSubmit" type="success">保存</el-button>
          </li>
        </ul>
      </div>
      <div class="content-right min-gutter">
        <div class="hide-content show-content">
          <el-form :model="form" :rules="rules" label-width="80px"
                   ref="d-form" style="padding-right: 20px">
            <el-form-item label="货主疫苗" prop="orgGoodsId">
              <el-select :clearable="true" :remote-method="filterOrgGoods" @change="orgGoodsChange" @click.native.once="filterOrgGoods('')"
                         filterable
                         placeholder="请输入名称或编号搜索货主疫苗" popper-class="good-selects"
                         remote v-model="form.orgGoodsId">
            ...
            <el-form-item label="批号" prop="batchNumberId">
              <el-select :remoteMethod="filterBatchNumber" @change="batchNumberChange" clearable filterable placeholder="请输入批号名称搜索批号"
                         remote v-model="form.batchNumberId">
            ...
            <el-form-item label="仓库" prop="warehouseId">
              <el-select @change="warehouseChange" clearable filterable
                         placeholder="请选择仓库"
            ...
            <div v-loading="loadingData">
              ...
              <el-row :key="item.id" class="md-info" v-for="item in batches">
                <el-col :span="14">
              ...
              <div class="md-inline-form" v-if="batches.length">
                <el-form-item class="pull-left" label="原状态" prop="adjustType">
                  <el-select clearable filterable placeholder="请选择原库存状态"
                             v-model="form.adjustType">
                    <el-option :key="item.key" :label="item.name" :value="item.key"
                               v-for="item in adjustTypeList" v-show="item.key !== 2">
                    </el-option>
                  </el-select>
                </el-form-item>
                <el-form-item class="pull-left" label="新状态" prop="adjustNewType">
                  <el-select clearable filterable placeholder="请选择新库存状态"
                             v-model="form.adjustNewType">
                    <el-option :key="item.key" :label="item.name" :value="item.key"
                               v-for="item in adjustTypeList" v-show="item.key !== form.adjustType">
                    </el-option>
                  </el-select>
                </el-form-item>
