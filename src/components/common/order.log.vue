  ...
  <div>
    ...
    <Timeline v-else>
      ...
      <template v-for="(log,index) in orderLogList">
        <TimelineItem v-if="log.showDate" color="green">
        ...
        <TimelineItem color="grey">
          <el-row>
            ...
            <el-col :span="18"><strong>{{log.actionTitle}}</strong>
              <el-tooltip v-show="log.operatorName" :content="log.operatorOrgName ? log.operatorOrgName : '平台用户' " class="item"
      ...
      <div v-if="orderLogList.length>0" class="order-cost-part">
        <i class="el-icon-time"></i> 订单消耗时间:
        <oms-cost-time :fDate="currentOrder.createTime" :tDate="orderEndTime"></oms-cost-time>
      </div>
    </Timeline>
