    ...
    <div class="content-part">
      <div class="content-left">
        ...
        <div class="product-list">
          <div v-show="!isValid" class="product-item is-total" style="cursor: default">
            <h2>基本信息:</h2>
            <oms-row :span="10" class="mb-10" label="包装是否完好">
              <el-radio-group v-model="form.packComplete">
                <el-radio :label="true">是</el-radio>
                <el-radio :label="false">否</el-radio>
              </el-radio-group>
            </oms-row>
            <oms-row :span="10" label="温度是否合格">
          ...
          <div v-show="isValid">
            <div v-for="(product,key) in productList" :class="{'active': activeKey === key }"
                 class="product-item" @click="changeProduct(product, key)">
              <oms-row :span="span" label="疫苗名称">{{product.name}}</oms-row>
              <oms-row :span="span" label="批号">{{product.batchNumber}}</oms-row>
              <oms-row :span="span" label="规格">{{product.orgGoodsDto.goodsDto.specifications}}</oms-row>
              <oms-row :span="span" label="生产单位">{{product.orgGoodsDto.goodsDto.factoryName}}</oms-row>
              <oms-row :span="span" label="数量">{{product.amount}}</oms-row>
            </div>
            <div class="product-item is-total" style="cursor: default">
              <h2>合计:</h2>
              <oms-row :span="span" label="应收数量">{{ totalCount }}</oms-row>
              <oms-row :span="span" label="实收数量">{{ receiptCount }}</oms-row>
            </div>
          </div>
        </div>
        <div v-show="isValid" class="btn-submit-save">
          <perm label="pov-receipt-manager">
            <el-button :disabled="doing" :plain="true" type="primary" @click="autoComplete">一键收货</el-button>
          </perm>
          <el-button :disabled="doing" type="primary" @click="onSubmit">保存</el-button>
        </div>
      </div>
      <div v-show="isValid" class="content-right content-padding">
        <h3></h3>
        <div v-for="(item, key) in productList" v-show="key === activeKey">
          <el-form :ref=" 'form' + key" :model="item" :rules="rules" label-width="160px"
            ...
            <el-form-item label="数量">
              <input v-model.number="item.currentAmount" class="el-input__inner" placeholder="请输入数量" type="number"
