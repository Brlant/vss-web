  ...
  export default {
    ...
    methods: {
      onSubmit() {
        ...
        this.doing = true;
        this.$http.post(`/erp-order/${this.orderId}/review/in`, codeList).then(res => {
          this.form.codeStr = '';
          this.doing = false;
          this.queryCodeList();
        }).catch(error => {
      ...
      queryCodeList() {
        this.$http.get(`/erp-order/in/${this.orderId}/code`).then(res => {
      ...
      passReview() {
        if (this.doing) return;
        this.doing = true;
        this.$http.post(`/erp-order/${this.orderId}/review/in/complete`).then(res => {
          this.$emit('refresh');
