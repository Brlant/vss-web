...
<template>
  <div v-show="currentItem.goodsId" class="part-count">
    <product-info :currentItem="currentItem"></product-info>
    <div v-for="(item) in receiptInfoList" class="product-batch-item">
      <oms-row :span="0" label="" style="margin-bottom: 10px">
        <h2>
          <span class="title">
            批号:{{ item.no }}
          ...
          <el-button plain size="mini" style="margin-left: 10px" type="danger" @click="deleteReceiptInfo(item)">删除
          </el-button>
          <el-button plain size="mini" type="warning" @click="edit(item)">编辑</el-button>
        </h2>
      </oms-row>
      <el-row>
        <el-col :span="9"
                style="margin-bottom: 10px">
          <oms-row :span="span" label="生产日期">{{ item.productionDate | date }}</oms-row>
          <oms-row :span="span" label="有效期">{{ item.expiryDate | date }}</oms-row>
          <oms-row :span="span" label="批签发号">{{ item.batchReleaseNumber }}</oms-row>
          <oms-row :span="span" label="批量/进口量">{{ item.batchReleaseCount }}</oms-row>
          <oms-row :span="span" label="批签发日期">{{ item.batchReleaseDate | date}}</oms-row>
        </el-col>
        <el-col :span="15">
          <oms-row :span="span" label="通关单号编码">{{ item.customsFormNumber }}</oms-row>
          <oms-row :span="span" label="口岸检验报告编号">{{ item.portInspectionNumber }}</oms-row>
          <oms-row v-show="item.smallPackageCount > 0 " :span="span" label="数量">
          ...
          <oms-row v-show="item.smallNonconformityCount > 0" :span="span" label="数量">
    ...
    <h2 v-show="receiptInfoList.length" class="text-right">
      实收合计:{{ totalCount }}
      <span v-if="currentItem.orgGoodsDto">
        <dict :dict-group="'measurementUnit'" :dict-key="currentItem.orgGoodsDto.goodsDto.measurementUnit"></dict>
      </span>
    ...
    <div v-show="!addBatchNumber">
      <el-form ref="batchForm" :model="form" label-width="110px">
        <el-form-item label="批号" prop="batchNumberId">
          ...
          <el-row>
            <el-col :span="12">
              <el-select ref="batchSelect" v-model="form.batchNumberId" :remoteMethod="filterBatchNumber" clearable
                         filterable placeholder="请输入批号名称搜索批号" remote
                         @blur="judgeAddBatchNumber" @change="changeBatchId" @clear="clearBatch">
        ...
        <batch-receipt v-if="isShowBatchReceipt" :currentItem="currentItem" :form="form"></batch-receipt>
        <div v-for="item in batchNumberInfos" v-show="form.batchNumberId && !isShowBatchReceipt" :keys="item.id">
          <el-row class="batch-info">
            <el-col :span="12">
              <oms-row :span="12" label="生产日期">{{ item.productionDate | date }}</oms-row>
              <oms-row :span="12" label="有效期">{{ item.expirationDate | date }}</oms-row>
              <oms-row :span="12" label="批签发号">{{ item.batchReleaseNumber }}</oms-row>
              <oms-row :span="12" label="批量/进口量">{{ item.batchReleaseCount }}</oms-row>
              <oms-row :span="12" label="批签发日期">{{ item.batchReleaseDate | date}}</oms-row>
            </el-col>
            <el-col :span="12">
              <oms-row :span="12" label="口岸检验报告编号">{{ item.portInspectionNumber }}</oms-row>
              <oms-row label="通关单号编码">{{ item.customsFormNumber }}</oms-row>
            </el-col>
          </el-row>
        </div>
        <el-form-item label="合格数量">
          <oms-input v-model="form.smallPackageCount" :min="0" placeholder="请输入" type="text">
            <template v-if="currentItem.orgGoodsDto" slot="append">
              <dict :dict-group="'measurementUnit'"
                    :dict-key="currentItem.orgGoodsDto.goodsDto.measurementUnit"></dict>
            </template>
          </oms-input>
        </el-form-item>
        <el-form-item class="nonconformity" label="不合格数量">
          <oms-input v-model="form.smallNonconformityCount" :min="0" placeholder="请输入" type="text">
            <template v-if="currentItem.orgGoodsDto" slot="append">
              <dict :dict-group="'measurementUnit'"
                    :dict-key="currentItem.orgGoodsDto.goodsDto.measurementUnit"></dict>
            </template>
        ...
        <el-form-item class="clearfix">
          <el-button :disabled="doing" type="primary" @click="submitConfirm">添加收货详情</el-button>
  ...
  methods: {
    ...
    deleteReceiptInfo(item) {
      this.$confirm('是否删除收货信息', '', {
        ...
        // }).catch(error => {
        //   this.$notify.error({
        //     message: error.response && error.response.data && error.response.data.msg || '删除收货信息失败'
        //   });
        // });
      });
    },
