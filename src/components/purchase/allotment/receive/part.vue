  ...
  <div>
    <div class="content-part">
      <div class="content-left">
        ...
        <ul class="show-list">
          <li class="list-item" v-for="(item, key) in products" @click="changeProduct(item, key)"
              :class="{ 'active' : index===key}">
            <div class="title">
              <span v-if="item.orgGoodsDto">{{ item.orgGoodsDto.goodsDto.name }}</span>
              <span v-else="">{{ item.name }}</span>
            </div>
            <div class="count" v-if="item.orgGoodsDto">
              应收数量: {{item.amount}}
          ...
          <li align="center" style="margin-top:40px;position:absolute;bottom:30px;left:0;right:0;text-align: center">
            <div v-show="isShowBtn">
              <div>
                <el-button type="success" @click="submitReceiptInfo()" :disabled="doing">
                  提交收货信息
                </el-button>
              </div>
            </div>
            <el-button style="padding: 10px 28px" type="primary"
                       @click="receiptOrder()" :disabled="doing" class="mt-10">收货完成
            </el-button>
          </li>
        </ul>
      </div>
      <div class="content-right min-gutter">
        <h3></h3>
        <batch-part :orderId="orderId" :currentOrder="currentOrder"
                    :currentItem="currentItem" :index="index" @showSubmitButton="showSubmitButton"></batch-part>
      </div>
    </div>
  </div>
</template>
