    ...
    <div v-else>
      <div v-for="exception in exceptionDtoList" :key="exception.id" @click="setItem(exception)">
        <div v-if="exception.flag">
          <bg-box :title="exception.item.name" class="bg-box-all" type="white">
            <div class="clearfix">
              ...
              <oms-row :span="4" class="mb-15" label="异常备注">{{exception.description}}</oms-row>
              <!--<oms-row :span="4" label="客户意见" class="mb-15">{{exception.dealDescriptionOrg}} </oms-row>-->

              <oms-row v-if="exception.reasonAttachments.length" :span="4" class="mb-15" label="异常原因附件">
        ...
        <div v-if="!exception.flag">
          <bg-box :title="'拒收信息'" class="bg-box-all" type="white">
            <div class="clearfix">
              ...
              <oms-row v-if="exception.reviewAttachments.length" :span="4" class="mb-15" label="拒收复核附件">
                <attachment-show :attachmentRight="reviewReasonRight"
                                 :orderAttachment="exception.reviewAttachments"
                                 attachmentClass="exception-attachment"
                                 @refreshAttachment="queryAllExceptionList"></attachment-show>
              </oms-row>
              <oms-row v-if="exception.feedBackAttachments.length" :span="4" class="mb-15" label="客户反馈附件">
