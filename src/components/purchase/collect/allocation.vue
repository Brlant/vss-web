  ...
  <div class="order-page">
    <div class="container">
      <div class="mb-15" style="overflow: hidden">
        <el-button :plain="true" class="pull-left" type="primary" @click="$router.push('/purchase/allocation')">返回采购汇总
        </el-button>
        <perm v-show="!$route.query.type" label="submit-pull-signal">
          <el-button v-show="status === 0 " :disabled="pullDoing" class="pull-right" style="margin-left:20px"
                     type="primary" @click="pullSubmit">生成要货申请
          </el-button>
        </perm>
        <perm v-show="!$route.query.type" label="submit-purchansing-plan">
          <el-button v-show="status === 0 " :disabled="doing" class="pull-right" type="primary" @click="submit">生成采购合同
          </el-button>
        </perm>
      </div>
          ...
          <div v-for="item in allocationList" :class="[{'active':currentItemId==item.orgGoodsId}]"
            ...
            <el-row>
              <el-col :span="6" class="R pt">
                <div>
                  <el-tooltip class="item" content="疫苗名称" effect="dark" placement="right">
